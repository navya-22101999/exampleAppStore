...
class AppStore extends Component {
  ...
  getActiveTabApps = searchedApps => {
    ...
    const filteredApps = searchedApps.filter(
      eachSearchedApp => eachSearchedApp.category === activateId,
    )
    return filteredApps
  }
  ...
  render() {
    const {searchInput, activateId} = this.state
          ...
          <ul className="tabs-list">
            {tabsList.map(eachTab => (
              <TabItem
                ...
                isActive={eachTab.tabId === activateId}
              />
            ))}
          </ul>
